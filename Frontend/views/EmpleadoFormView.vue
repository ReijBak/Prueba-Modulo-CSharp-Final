    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-gray-600">
          {{ isEditing ? 'Actualiza la información del empleado' : 'Registra un nuevo empleado en el sistema' }}
        </p>
      </div>
      <router-link
        to="/empleados"
    ...
    <form v-if="!loading" @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow-md p-6">
      <div class="grid grid-cols-1 gap-6" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));">
        ...
        <div>
          ...
          >
            <option value="">Seleccionar...</option>
            <option v-for="dep in catalogs.departamentos" :key="dep.id" :value="dep.id">
              {{ dep.nombre }}
            </option>
          </select>
        </div>
        ...
        <!-- Cargo -->
        <div>
          <label for="cargoId" class="block text-sm font-medium text-gray-700 mb-2">
            Cargo *
          </label>
          <select
            id="cargoId"
            v-model.number="form.cargoId"
            required
            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
            <option value="">Seleccionar...</option>
            <option v-for="cargo in catalogs.cargos" :key="cargo.id" :value="cargo.id">
              {{ cargo.nombre }}
            </option>
          </select>
        </div>
        ...
        <!-- Nivel Educativo -->
        <div>
          <label for="nivelEducativoId" class="block text-sm font-medium text-gray-700 mb-2">
            Nivel Educativo *
          </label>
          <select
            id="nivelEducativoId"
            v-model.number="form.nivelEducativoId"
            required
            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
            <option value="">Seleccionar...</option>
            <option v-for="nivel in catalogs.nivelesEducativos" :key="nivel.id" :value="nivel.id">
              {{ nivel.nombre }}
            </option>
          </select>
        </div>
        ...
        <div style="grid-column: 1 / -1;">
          <label for="perfilProfesional" class="block text-sm font-medium text-gray-700 mb-2">
            Perfil Profesional
          </label>
          <textarea
            id="perfilProfesional"
            v-model="form.perfilProfesional"
            rows="4"
            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ></textarea>
        </div>
      ...
      <div class="mt-6 flex justify-end space-x-4">
        <router-link
          to="/empleados"
          class="px-6 py-2 bg-gray-200 text-gray-700 font-medium rounded-md hover:bg-gray-300"
        >
          Cancelar
        </router-link>
        <button
