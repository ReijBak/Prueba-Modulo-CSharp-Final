  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-100 py-12 px-4">
    <div class="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
      ...
      <form v-else @submit.prevent="handleEmployeeLogin" class="space-y-6">
        ...
        <div>
          <label for="emp-password" class="block text-sm font-medium text-gray-700 mb-2">Contraseña</label>
          <input
            id="emp-password"
...
const employeeForm = reactive({ documento: '', password: '' })
...
const handleEmployeeLogin = async () => {
  ...
  try {
    const response = await authService.employeeLogin(parseInt(employeeForm.documento), employeeForm.password)
    if (response.success) {
      authStore.setAuth(response)
