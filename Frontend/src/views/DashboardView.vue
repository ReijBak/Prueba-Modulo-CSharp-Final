  ...
  <div class="space-y-6">
    ...
    <div v-if="result" class="bg-white rounded-lg shadow-md p-6">
      ...
      <div v-if="result.result && result.result.length > 0" class="overflow-x-auto">
        <p class="text-sm font-medium text-gray-700 mb-2">Resultados ({{ result.result.length }} registros):</p>
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th v-for="key in Object.keys(result.result[0])" :key="key" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                {{ key }}
              </th>
            </tr>
          </thead>
